...
transform_and_plot(nc, new_crs = "EPSG:3112")
transform_and_plot(nc, new_crs = "EPSG:6684")
...
> If the point location data were in a different CRS from the original map, I would expect the points to be inaccurately positioned if they were not transformed.
...
> The viridis scale uses plots that can be analyzed both in color and in black and white, and they will also be colorblind friendly as a result. _d() is used for categorical variables, _c() is used for continuous variables, and _b() is used for binned continuous variables.
...
ggplot() +
    ...
    labs(title = "Twin Cities wfith Population >= 10,000") + 
    ggthemes::theme_map() +
...
ggsave("../figures/tc_map_zoom.png", width = 865, height = 1400, units = "px")
```
